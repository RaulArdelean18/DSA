...
void testCreeaza() {
    ...
    IteratorDictionar id = d.iterator(); // iterator pe multimea vida ar trebui
                                         // sa fie invalid din start
...
void testAdauga() {
    ...
    for (int i = 10000; i > -10000;
         i--) { // adaugam mult, si acum prima data adaugam valori mari, dupa
                // aceea mici
        d.adauga(i, i);
    }
    assert(d.dim() == 20000);
...
void testSterge() {
    ...
    // printD(d);
    for (int i = 100; i > -100; i--) { // stergem descrescator (in ordine
                                       // inversa fata de ordinea adaugarii)
        if (i % 2 == 0) {
            assert(d.sterge(i) == i);
        } else {
            assert(d.sterge(i) == NULL_TVALOARE);
    ...
    assert(d.dim() == 200);
    for (int i = -200; i < 200;
         i++) { // stergem elemente inexistente si existente
        if (i < -100 || i >= 100) {
            assert(d.sterge(i) == NULL_TVALOARE);
        } else {
            assert(d.sterge(i) == 4);
...
void testIterator() {
    ...
    IteratorDictionar id2 =
        d.iterator(); // daca iterez doar 33 poate sa-mi dea iteratorul
    ...
    Dictionar d2;
    for (int i = -100; i < 100; i++) { // adaug 200 de elemente, fiecare de 3
                                       // ori
        d2.adauga(i, i);
        d2.adauga(i, i);
        d2.adauga(i, i);
    ...
    int count = 0;
    while (id4.valid()) { // fiecare element e multiplu de 4 si sunt in total 50
                          // de elemente
        TElem e = id4.element();
        assert(e.first % 4 == 0);
        id4.urmator();
...
void testQuantity() { // scopul e sa adaugam multe date
    ...
    id.prim();
    while (id.valid()) { // fiecare element returnat de iterator trebuie sa fie
                         // in container
        TElem e = id.element();
...
// nu stim reprezentarea, putem testa doar anumite lucruri generale, nu stim in
// ce ordine vor fi afisate elementele
void testAllExtins() {
    testCreeaza();
    testAdauga();
    testSterge();
